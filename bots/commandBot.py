...
# don't fuck with this too much unless youre familiar with twitchio and how it works
...
# starts command bot in a new thread
def startCommandBot():
    bot = Bot()
...
class Bot(commands.Bot):
    ...
    # allows mods to ping sna1l if something goes wrong with the code
    @commands.command()
    async def panicCode(self, ctx: commands.Context):
        ...
        # updates and writes the variable in the config that tells the discord bot to send the messages
    ...
    # allows mods to ping fizz if something goes wrong with the stream
    @commands.command()
